      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }, onMouseDown: function (e) { var t = e.target; if (t && t.closest && (t.closest('[data-cell]') || t.closest('button,input,select'))) return; clearSelection() } },
        ...
        colorMenu ? h('div', {
          ...
          children: [
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, borderTop: '1px solid #eee', paddingTop: 4 } },
              h('span', { style: { fontSize: 11, color: '#888' } }, '自定义'),
              h('input', {
                type: 'color',
                defaultValue: '#000000',
                onChange: function (e) {
                  if (sel && sel.r !== undefined && sel.r >= 0) {
                    updateStyle(sel.r, sel.c, colorMenu.type === 'font' ? { color: hexToArgb(e.target.value) } : { bg: hexToArgb(e.target.value) })
                  }
                  setColorMenu(null)
                },
                style: { width: 28, height: 22, padding: 0, border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer', background: '#fff' }
              })
            )
          ]
        }) : null,
