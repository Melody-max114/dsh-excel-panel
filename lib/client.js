window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PICKER_COLORS = ['#000000', '#FFFFFF', '#FF0000', '#FFA500', '#FFFF00', '#008000', '#0000FF', '#800080', '#808080', '#FFC0CB', '#A52A2A', '#FFD700', '#00FF00', '#00FFFF', '#000080', '#FF00FF', '#F0F8FF', '#FAEBD7', '#2F4F4F', '#8B0000', '#FF8C00', '#FF69B4', '#7CFC00', '#00CED1', '#483D8B', '#8B008B', '#D3D3D3', '#F5F5DC', '#B22222', '#DAA520', '#228B22', '#008B8B', '#191970', '#9400D3', '#FFF8DC', '#FFE4E1', '#556B2F', '#8B4513', '#4682B4', '#C71585']
            ...
            h('div', { style: { display: 'flex', alignItems: 'center', gap: 4, marginTop: 6, borderTop: '1px solid #eee', paddingTop: 4 } },
              ...
              customColors.map(function (color, i) {
                return h('button', {
                  ...
                  style: { width: 18, height: 18, border: '1px solid #ccc', borderRadius: 3, cursor: 'pointer', background: (customColorIndex === i && pendingCustomColor) ? pendingCustomColor : color, padding: 0 }
                })
              }),
              customColorIndex !== null ? h('div', { style: { marginTop: 6, borderTop: '1px solid #eee', paddingTop: 6, width: '100%' } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 } },
                  h('span', { style: { fontSize: 11, color: '#888' } }, '选色'),
                  h('span', { style: { width: 20, height: 20, border: '1px solid #ccc', borderRadius: 3, background: pendingCustomColor || customColors[customColorIndex] || '#FFFFFF', display: 'inline-block' } }),
                  h('button', {
                    onClick: function () {
                      var picked = pendingCustomColor || customColors[customColorIndex]
                    ...
                    title: '确认自定义颜色',
                    style: { width: 22, height: 22, border: '1px solid #165dff', borderRadius: 3, cursor: 'pointer', background: '#eef2ff', color: '#165dff', fontWeight: 'bold', padding: 0, fontSize: 14, lineHeight: '20px' }
                  }, '✓')
                ),
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(8, 18px)', gap: 3 } },
                  PICKER_COLORS.map(function (pc) {
                    return h('button', {
                      key: pc,
                      onClick: function () { setPendingCustomColor(pc) },
                      style: { width: 18, height: 18, border: '1px solid #ccc', borderRadius: 2, cursor: 'pointer', background: pc, padding: 0 }
                    })
                  })
                ),
                h('div', { style: { display: 'flex', flexDirection: 'column', gap: 2, marginTop: 4 } },
                  ['R', 'G', 'B'].map(function (ch, ci) {
                    var rgb = hexToRgb(pendingCustomColor || customColors[customColorIndex] || '#FFFFFF')
                    var val = ci === 0 ? rgb.r : ci === 1 ? rgb.g : rgb.b
                    return h('label', { key: ch, style: { display: 'flex', alignItems: 'center', gap: 4, fontSize: 11 } },
                      h('span', { style: { width: 12 } }, ch),
                      h('input', { type: 'range', min: 0, max: 255, value: val, onChange: function (e) {
                        var cur = hexToRgb(pendingCustomColor || customColors[customColorIndex] || '#FFFFFF')
                        var nv = Number(e.target.value)
                        var next = ci === 0 ? { r: nv, g: cur.g, b: cur.b } : ci === 1 ? { r: cur.r, g: nv, b: cur.b } : { r: cur.r, g: cur.g, b: nv }
                        setPendingCustomColor(rgbToHex(next.r, next.g, next.b))
                      }, style: { flex: 1 } }),
                      h('span', { style: { width: 28, textAlign: 'right' } }, val)
                    )
                  }),
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 } },
                    h('span', { style: { fontSize: 11, color: '#888' } }, 'HEX'),
                    h('input', { type: 'text', value: pendingCustomColor || customColors[customColorIndex] || '#FFFFFF', onChange: function (e) { var v = e.target.value; if (/^#[0-9a-fA-F]{6}$/.test(v)) setPendingCustomColor(v) }, style: { flex: 1, border: '1px solid #ccc', borderRadius: 3, padding: '2px 4px', fontSize: 11 } })
                  )
                )
              ) : null
            )
