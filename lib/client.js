window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function XlsxEditor({ scope, path, title, customData }) {
      ...
      React.useEffect(function () {
        ...
        function onMove(e) {
          if (resize.type === 'col') {
            ...
            updateSheetSize('col', resize.index, resize.startSize + dx)
          } else {
            var dy = e.clientY - resize.startY
            updateSheetSize('row', resize.index, resize.startSize + dy)
          }
        }
      ...
      React.useEffect(function () {
        if (data.status !== 'ready' || data.saveState === 'dirty' || data.saveState === 'saving' || data.saveState === 'error') return
      ...
      React.useEffect(function () {
        if (data.status !== 'ready' || data.saveState !== 'dirty') return
        var timer = setTimeout(function () { save() }, 1200)
