  ...
  factory: (require) => {
    ...
    function cloneRows(rows) {
      return rows.map(function (row) {
        return row.map(function (cell) {
          return { v: cell.v, f: cell.f, bg: cell.bg, font: cell.font }
        })
      })
    }
    ...
    function XlsxEditor({ scope, path, title, customData }) {
      ...
      React.useEffect(function () {
        ...
        var cancelled = false
        readOffice('xlsx', scope, path).then(function (wb) {
          if (cancelled) return
      ...
      function updateCell(r, c, text) {
        ...
        var sheets = wb.sheets.map(function (sheet) {
          ...
          rows[r][c] = { v: formatCellValue(value), f: formula, bg: prev && prev.bg, font: prev && prev.font }
          recalcSheet({ name: sheet.name, rows: rows })
          return { name: sheet.name, rows: rows, merges: sheet.merges }
        })
      ...
      function moveSelection(dr, dc) {
        ...
        var maxR = Math.max(0, sheetData.rows.length - 1)
        var maxC = 0
        for (var i = 0; i < sheetData.rows.length; i++) maxC = Math.max(maxC, sheetData.rows[i].length - 1)
        var nr = Math.max(0, Math.min(maxR, sel.r + dr))
        var nc = Math.max(0, Math.min(maxC, sel.c + dc))
        selectCell(nr, nc)
        setRange(null)
        setAnchor({ r: nr, c: nc })
      }
      ...
      function mergeRange(selRange) {
        ...
        var sheets = wb.sheets.map(function (sheet) {
          ...
          return { name: sheet.name, rows: sheet.rows, merges: merges }
        })
        setHistory(function (h) { return { past: h.past.concat([wb]), future: [] } })
        setData({ ...data, workbook: { ...wb, sheets: sheets }, saveState: 'dirty' })
        setRange(null)
      ...
      function handleCellMouseDown(e, r, c) {
        ...
        if (e.shiftKey && anchor) {
          setRange({ r1: Math.min(anchor.r, r), c1: Math.min(anchor.c, c), r2: Math.max(anchor.r, r), c2: Math.max(anchor.c, c) })
          selectCell(r, c)
        } else {
          setAnchor({ r: r, c: c })
          setRange(null)
      ...
      function insertRow(r) {
        ...
        var sheets = wb.sheets.map(function (sheet) {
          ...
          return { name: sheet.name, rows: rows, merges: merges }
        })
        setHistory(function (h) { return { past: h.past.concat([wb]), future: [] } })
        setData({ ...data, workbook: { ...wb, sheets: sheets }, saveState: 'dirty' })
        closeMenu()
      ...
      React.useEffect(function () {
        function onWindowMouseDown() { setMenu(null) }
      ...
      React.useEffect(function () {
        ...
        function onMove(e) {
          ...
          if (Number.isFinite(r) && Number.isFinite(c)) {
            setRange({ r1: Math.min(rangeDrag.anchor.r, r), c1: Math.min(rangeDrag.anchor.c, c), r2: Math.max(rangeDrag.anchor.r, r), c2: Math.max(rangeDrag.anchor.c, c) })
      ...
      function applyStyleToWorkbook(wb, r, c, patch) {
        var rangeToApply = range && range.r1 !== undefined
          ...
          for (var rr = rangeToApply.r1; rr <= rangeToApply.r2; rr++) {
            for (var cc = rangeToApply.c1; cc <= rangeToApply.c2; cc++) {
              ...
              if (Object.prototype.hasOwnProperty.call(patch, 'bg')) {
                rows[rr][cc] = { v: prev.v, f: prev.f, bg: patch.bg, font: prev.font }
              } else {
                var font = Object.assign({}, prev.font || {}, patch)
                rows[rr][cc] = { v: prev.v, f: prev.f, bg: prev.bg, font: font }
              }
            }
          }
          return { name: sheet.name, rows: rows }
        })
          ...
          for (var r = minR; r <= maxR; r++) {
            for (var c = minC; c <= maxC; c++) {
              ...
              if (srcCell.f) {
                ...
                rows[r][c] = { v: evaluateFormula('=' + shifted, rows, r, c, []), f: shifted, bg: rows[r][c] && rows[r][c].bg, font: srcCell.font }
              } else {
                rows[r][c] = { v: srcCell.v, f: undefined, bg: rows[r][c] && rows[r][c].bg, font: srcCell.font }
              }
            }
          }
          recalcSheet({ name: sheet.name, rows: rows })
      ...
      React.useEffect(function () {
        function onKeyDown(e) {
          ...
          if (!mod) {
            ...
            if (e.key === 'Enter') {
              e.preventDefault()
              if (sel && sel.r !== undefined) startEdit(sel.r, sel.c)
              return
            }
            if (e.key === 'Tab') {
              e.preventDefault()
      ...
      React.useEffect(function () {
        if (data.status !== 'ready' || data.saveState === 'dirty' || data.saveState === 'saving') return
        var timer = setInterval(function () {
          readExcel(scope, path).then(function (wb) {
            setData(function (prev) {
              if (prev.status !== 'ready' || prev.saveState === 'dirty' || prev.saveState === 'saving') return prev
              var active = prev.activeSheet && wb.sheets.some(function (s) { return s.name === prev.activeSheet })
                ? prev.activeSheet
                : (wb.activeSheet || (wb.sheets[0] && wb.sheets[0].name) || 'Sheet1')
              var next = { ...prev, workbook: wb, activeSheet: active, saveState: 'idle' }
              return JSON.stringify(prev.workbook) === JSON.stringify(wb) ? prev : next
            })
          }).catch(function () {})
        }, 2000)
        return function () { clearInterval(timer) }
      }, [scope.cwd, path, data.status, data.saveState])
      ...
      function save() {
        ...
        post('/excel-panel/write', { root: target.root, path: target.path, kind: 'xlsx', payload: { sheets: data.workbook.sheets } }).then(function (res) {
          if (!res.ok) throw new Error((res.error && res.error.message) || 'save failed')
          setData({ ...data, saveState: 'saved' })
        }).catch(function (e) {
          setData({ ...data, saveState: 'error', message: String(e.message || e) })
        })
      }
      ...
      var rows = sheet.rows || []
      var colCount = Math.max(1, rows.reduce(function (m, row) { return Math.max(m, row.length) }, 0))
      var rowCount = Math.max(1, rows.length)
      var selCell = (rows[sel.r] && rows[sel.r][sel.c]) || { v: '' }
      var selLabel = (sel && sel.r !== undefined) ? cellAddress(sel.r, sel.c) : ''
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' } },
        ...
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, padding: '3px 8px', borderBottom: '1px solid #ddd', background: '#fff', fontSize: 12 } },
          h('select', {
            value: (selCell.font && selCell.font.size) || 11,
            onChange: function (e) { if (sel && sel.r !== undefined) updateStyle(sel.r, sel.c, { size: Number(e.target.value) }) },
            style: { padding: '2px 4px', border: '1px solid #ccc', borderRadius: 4 }
          }, [9, 10, 11, 12, 14, 16, 18, 20].map(function (n) { return h('option', { key: n, value: n }, n) })),
          h('button', {
            onClick: function () { if (sel && sel.r !== undefined) updateStyle(sel.r, sel.c, { bold: !(selCell.font && selCell.font.bold) }) },
            style: { padding: '2px 8px', cursor: 'pointer', fontWeight: 'bold', border: selCell.font && selCell.font.bold ? '1px solid #165dff' : '1px solid #ccc', borderRadius: 4, background: selCell.font && selCell.font.bold ? '#eef2ff' : '#fff' }
          }, 'B'),
          h('button', {
            onClick: function () { if (sel && sel.r !== undefined) updateStyle(sel.r, sel.c, { italic: !(selCell.font && selCell.font.italic) }) },
            style: { padding: '2px 8px', cursor: 'pointer', fontStyle: 'italic', border: selCell.font && selCell.font.italic ? '1px solid #165dff' : '1px solid #ccc', borderRadius: 4, background: selCell.font && selCell.font.italic ? '#eef2ff' : '#fff' }
          }, 'I'),
          h('button', {
            onClick: function () { if (sel && sel.r !== undefined) updateStyle(sel.r, sel.c, { underline: !(selCell.font && selCell.font.underline) }) },
            style: { padding: '2px 8px', cursor: 'pointer', textDecoration: 'underline', border: selCell.font && selCell.font.underline ? '1px solid #165dff' : '1px solid #ccc', borderRadius: 4, background: selCell.font && selCell.font.underline ? '#eef2ff' : '#fff' }
          }, 'U'),
          h('input', {
            type: 'color',
            value: selCell.font && selCell.font.color ? argbToHex(selCell.font.color) : '#000000',
            onChange: function (e) {
              if (sel && sel.r !== undefined) {
                if (!colorStartRef.current) colorStartRef.current = data.workbook
                updateStyleNoHistory(sel.r, sel.c, { color: hexToArgb(e.target.value) })
              }
            },
            onBlur: function () {
              if (colorStartRef.current) {
                var startWb = colorStartRef.current
                colorStartRef.current = null
                setHistory(function (h) { return { past: h.past.concat([startWb]), future: [] } })
              }
            },
            title: '字体颜色',
            style: { width: 26, height: 22, padding: 0, border: '1px solid #ccc', borderRadius: 4, cursor: 'pointer', background: '#fff' }
          }),
          h('input', {
            type: 'color',
            value: selCell.bg ? argbToHex(selCell.bg) : '#ffffff',
            onChange: function (e) {
              if (sel && sel.r !== undefined) {
                if (!colorStartRef.current) colorStartRef.current = data.workbook
                updateStyleNoHistory(sel.r, sel.c, { bg: hexToArgb(e.target.value) })
              }
            },
            onBlur: function () {
              if (colorStartRef.current) {
          ...
          h('button', {
            onClick: function () { if (range) mergeRange(range) },
            title: '合并选中的区域（Shift+点击选择范围）',
            style: { padding: '2px 8px', cursor: 'pointer', border: '1px solid #ccc', borderRadius: 4, background: '#fff', color: '#333' }
          }, '合并'),
          h('button', {
            onClick: function () { if (sel && sel.r !== undefined) unmergeAt(sel.r, sel.c) },
            title: '取消当前所在合并区域',
            style: { padding: '2px 8px', cursor: 'pointer', border: '1px solid #ccc', borderRadius: 4, background: '#fff', color: '#333' }
          }, '取消合并')
        ),
          ...
          h('table', { style: { borderCollapse: 'collapse', fontSize: 13 } },
            h('tbody', null,
              h('tr', null,
                ...
                Array.from({ length: colCount }, function (_, c) { return h('td', { key: c, style: headerStyle() }, colName(c + 1)) })
              ),
              Array.from({ length: rowCount }, function (_, r) {
                var row = rows[r] || []
                return h('tr', { key: r },
                  h('td', { style: headerStyle() }, r + 1),
                  Array.from({ length: colCount }, function (_, c) {
                    ...
                    var tdStyle = {
                      ...
                      background: isSelected ? '#eef2ff' : (inRange ? 'rgba(22,93,255,0.25)' : (bg || undefined)),
                      boxShadow: isSelected ? 'inset 0 0 0 2px #165dff' : (inRange ? 'inset 0 0 0 1px rgba(22,93,255,0.55)' : undefined),
                    ...
                    return h('td', {
                      ...
                      children: [
                        editing && editing.r === r && editing.c === c
                          ? h('input', {
                              style: Object.assign({ width: '100%', minWidth: 70, border: 'none', padding: '2px 6px', background: 'transparent', font: 'inherit', outline: 'none' }, fontStyle, { color: fontStyle.color !== 'inherit' ? fontStyle.color : (isFormula ? '#165dff' : 'inherit'), fontWeight: fontStyle.fontWeight !== 'inherit' ? fontStyle.fontWeight : (isFormula ? 600 : 'inherit') }),
                              value: displayValue,
                              autoFocus: true,
                              onChange: function (e) {
                          ...
                          : h('div', {
                              style: Object.assign({ width: '100%', minWidth: 70, padding: '2px 6px', boxSizing: 'border-box', cursor: 'cell' }, fontStyle, { color: fontStyle.color !== 'inherit' ? fontStyle.color : (isFormula ? '#165dff' : 'inherit'), fontWeight: fontStyle.fontWeight !== 'inherit' ? fontStyle.fontWeight : (isFormula ? 600 : 'inherit') }),
        ...
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderTop: '1px solid #ddd', background: '#f8f9fa', fontSize: 12 } },
          wb.sheets.map(function (s) {
            return h('button', {
              key: s.name,
              onClick: function () { setData({ ...data, activeSheet: s.name, saveState: data.saveState }); setEditing(null); setSel({ r: 0, c: 0 }); setBarValue(''); setRange(null); setAnchor(null); setRangeDrag(null) },
        ...
        data.saveState === 'error' ? h('div', { style: { padding: '4px 8px', color: '#c00', fontSize: 12 } }, data.message) : null,
        menu ? h('div', {
