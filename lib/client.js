window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function XlsxEditor({ scope, path, title, customData }) {
      ...
      React.useEffect(function () {
        if (data.status !== 'ready' || !data.workbook) return
        if (!prevWorkbookRef.current) { prevWorkbookRef.current = data.workbook; return }
        if (JSON.stringify(prevWorkbookRef.current) === JSON.stringify(data.workbook)) return
        var actionInfo = lastActionRef.current || { action: 'change', time: new Date().toLocaleTimeString() }
        var entry = Object.assign({}, actionInfo, { time: new Date().toLocaleTimeString(), snapshot: data.workbook })
        setOpLog(function (prev) { return [entry].concat(prev).slice(0, 50) })
        var target = officeTarget(scope, path)
        post('/excel-panel/log', { root: target.root, path: target.path, op: entry }).catch(function () {})
        prevWorkbookRef.current = data.workbook
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', borderBottom: '1px solid #ddd', fontSize: 12 } },
          h('span', { style: { fontWeight: 600 } }, title + ' v0.4.0'),
          h('span', { style: { flex: 1 } }),
    ...
    function apply(ctx) {
      console.log('[dsh-excel-panel] v0.4.0 loaded')
