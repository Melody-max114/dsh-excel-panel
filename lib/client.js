  ...
  factory: (require) => {
    ...
    function XlsxEditor({ scope, path, title, customData }) {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }, onMouseDown: function (e) { var t = e.target; if (t && t.closest && (t.closest('[data-cell]') || t.closest('button,input,select'))) return; clearSelection() } },
        ...
        colorMenu ? h('div', {
          style: { position: 'fixed', zIndex: 9999, top: colorMenu.y, left: colorMenu.x, background: '#fff', border: '1px solid #ccc', borderRadius: 6, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', padding: 6, display: 'grid', gridTemplateColumns: 'repeat(4, 20px)', gap: 4 },
          onMouseDown: function (e) { e.stopPropagation() }
        }, (colorMenu.type === 'font' ? ['#000000', '#FFFFFF', '#FF0000', '#FFA500', '#FFFF00', '#008000', '#0000FF', '#800080'] : ['#FFFFFF', '#FFFF00', '#FFA500', '#FF0000', '#00B050', '#00B0F0', '#0000FF', '#7030A0']).map(function (color) {
          return h('button', {
            key: color,
            onClick: function () {
              if (sel && sel.r !== undefined && sel.r >= 0) {
