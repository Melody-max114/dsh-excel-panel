    ...
    function XlsxEditor({ scope, path, title, customData }) {
      ...
      return h('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }, onMouseDown: function (e) { var t = e.target; if (t && t.closest && (t.closest('[data-cell]') || t.closest('button,input,select'))) return; clearSelection() } },
        ...
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 6, padding: '3px 8px', borderBottom: '1px solid #ddd', background: '#fff', fontSize: 12 } },
          ...
          h('span', { style: { color: '#888', marginLeft: 4 } }, '字色'),
          ['#000000', '#FFFFFF', '#FF0000', '#FFA500', '#FFFF00', '#008000', '#0000FF', '#800080'].map(function (color) {
            return h('button', {
              key: 'f' + color,
              onClick: function () { if (sel && sel.r !== undefined && sel.r >= 0) updateStyle(sel.r, sel.c, { color: hexToArgb(color) }) },
              title: '字体颜色 ' + color,
              style: { width: 16, height: 16, borderRadius: 3, border: '1px solid #ccc', cursor: 'pointer', background: color, padding: 0 }
            })
          }),
          h('span', { style: { color: '#888', marginLeft: 4 } }, '底色'),
          ['#FFFFFF', '#FFFF00', '#FFA500', '#FF0000', '#00B050', '#00B0F0', '#0000FF', '#7030A0'].map(function (color) {
            return h('button', {
              key: 'b' + color,
              onClick: function () { if (sel && sel.r !== undefined && sel.r >= 0) updateStyle(sel.r, sel.c, { bg: hexToArgb(color) }) },
              title: '背景颜色 ' + color,
              style: { width: 16, height: 16, borderRadius: 3, border: '1px solid #ccc', cursor: 'pointer', background: color, padding: 0 }
            })
          }),
